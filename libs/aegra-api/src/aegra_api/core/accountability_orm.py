...
class DiscoveredOpportunity(Base):
    ...
    location: Mapped[str | None] = mapped_column(Text, nullable=True)
    event_date: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    company: Mapped[str | None] = mapped_column(Text, nullable=True)
    salary_range: Mapped[str | None] = mapped_column(Text, nullable=True)
    match_score: Mapped[Decimal | None] = mapped_column(Numeric(precision=3, scale=2), nullable=True)  # 0.00 - 1.00
    matched_track: Mapped[str | None] = mapped_column(Text, nullable=True)  # Track that triggered match
    status: Mapped[str] = mapped_column(
...
class UserActivityTracking(Base):
    ...
    last_conversation: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    last_course_activity: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    last_action_completed: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    current_streak: Mapped[int] = mapped_column(Integer, server_default=text("0"), nullable=False)
    longest_streak: Mapped[int] = mapped_column(Integer, server_default=text("0"), nullable=False)
